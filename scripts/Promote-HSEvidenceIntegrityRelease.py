...
for cid,r in q.items():
    ...
    if dest.exists(): shutil.copy2(dest,dest.with_suffix(dest.suffix+".pre-hs-evidence-integrity-release.bak"))
    payload={"evidence_id":e["evidence_id"],"control_id":cid,"scope":"EMS","authority_id":"EMS-GOVERNANCE","status":"PASS","evidence_date":e["evidence_date"],"expires_on":None,"evidence_summary":f"HS-EVIDENCE-INTEGRITY-RELEASE satisfied all required assertions for {cid}.","evidence_references":e["evidence_sources"],"notes":"collector_id=HS-EVIDENCE-INTEGRITY-RELEASE; collector_version=2B2.2"}
