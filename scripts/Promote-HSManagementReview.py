...
for cid,r in q.items():
    ...
    e=env[cid];folder=Path(a.evidence_root)/("ems" if e["scope"]=="EMS" else "organization");folder.mkdir(parents=True,exist_ok=True)
    dest=folder/f"{cid}.yaml"
    if dest.exists():shutil.copy2(dest,dest.with_suffix(dest.suffix+".pre-hs-management-review.bak"))
    payload={"evidence_id":e["evidence_id"],"control_id":cid,"scope":e["scope"],"authority_id":"EMS-GOVERNANCE" if e["scope"]=="EMS" else "ENGINEERING-LEADERSHIP","status":"PASS","evidence_date":e["evidence_date"],"expires_on":None,"evidence_summary":f"HS-MANAGEMENT-REVIEW satisfied all required assertions for {cid}.","evidence_references":e["evidence_sources"],"notes":"collector_id=HS-MANAGEMENT-REVIEW; collector_version=2B2.4"}
    dest.write_text(yaml.safe_dump(payload,sort_keys=False),encoding="utf-8");prom.append({"control_id":cid,"scope":e["scope"],"destination":str(dest)})
