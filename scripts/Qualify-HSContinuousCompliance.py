...
for e in rows:
    ...
    if not ok:g.append({"control_id":e["control_id"],"control_name":e["control_name"],"status":e["status"],"gap_reason":"CONTINUOUS_COMPLIANCE_ASSERTIONS_INCOMPLETE"})
with (out/"qualified_continuous_compliance.csv").open("w",newline="",encoding="utf-8") as f:
    w=csv.DictWriter(f,fieldnames=list(q[0].keys()));w.writeheader();w.writerows(q)
with (out/"continuous_compliance_gaps.csv").open("w",newline="",encoding="utf-8") as f:
