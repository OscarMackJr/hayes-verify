...
if len(rows)!=9:
    raise SystemExit(f"Review blocked: expected 9 assertions, found {len(rows)}")
for r in rows:
...
registers=[]
for name in ["approved_ai_platforms.csv","prompt_data_classification.csv","ai_security_reviews.csv"]:
    p=regroot/name
    if not p.exists():
