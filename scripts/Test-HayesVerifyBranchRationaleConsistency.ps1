...
Write-Host "=== Ruff rationale consistency hotfix ===" -ForegroundColor Cyan
& $py -m ruff check "$root\src" "$root\tests"
if($LASTEXITCODE){
    throw "Ruff failed after rationale consistency hotfix."
}

Write-Host "`n=== Pytest rationale consistency hotfix ===" -ForegroundColor Cyan
& $py -m pytest --basetemp="$pytestTemp" -q "$root\tests"
if($LASTEXITCODE){
    throw "Pytest failed after rationale consistency hotfix."
}

Write-Host "PASS: evaluated-branch rationale consistency validated." -ForegroundColor Green
