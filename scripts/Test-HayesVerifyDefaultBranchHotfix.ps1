...
if($LASTEXITCODE){
    throw "Ruff failed after default-branch hotfix."
}

Write-Host "`n=== Pytest deterministic repository-local gate ===" -ForegroundColor Cyan
& $py -m pytest --basetemp="$pytestTemp" -q
if($LASTEXITCODE){
    throw "Pytest failed after default-branch hotfix."
}

Write-Host "PASS: default branch discovery hotfix validated with deterministic repo-local pytest temp." -ForegroundColor Green
