...
EXPECTED={"EMS-CTRL-005","EMS-CTRL-016","EMS-CTRL-071","EMS-CTRL-072"}
...
for r in rows:
    if r["collector_id"]!="HS-EVIDENCE-INTEGRITY-RELEASE": errors.append(f"{r['control_id']}: wrong collector")
    ...
    if r["promotion_candidate"] and r["status"]!="PASS": errors.append(f"{r['control_id']}: invalid promotion_candidate")
