...
if registry.get("registry_version") != "1.5":
    raise SystemExit("expected registry_version=1.5")
...
if len(implemented) != 4:
    raise SystemExit(
        f"expected 4 implemented Priority-2 controls after v1.5, found {len(implemented)}"
    )
if len(remaining) != 17:
    raise SystemExit(
        f"expected 17 remaining Priority-2 controls, found {len(remaining)}"
    )
if "dependency_supply_chain" in groups:
    raise SystemExit(
        "dependency_supply_chain still appears in remaining Priority-2 backlog"
    )
