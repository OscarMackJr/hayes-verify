...
def test_cli_assess_status_and_resume_surface(tmp_path:Path)->None:
    ...
    command=[sys.executable,'-m','hayes_verify.archive_assessment','assess',str(archive),'--output-root',str(tmp_path/'out'),'--non-production','--json']
...
def test_cli_requires_non_production(tmp_path:Path)->None:
    archive=tmp_path/'archive.zip'
    with zipfile.ZipFile(archive,'w') as z:z.writestr('x.txt','x')
    result=subprocess.run([sys.executable,'-m','hayes_verify.archive_assessment','assess',str(archive),'--output-root',str(tmp_path/'out')],capture_output=True,text=True,check=False)
